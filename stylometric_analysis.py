...
class AcademicStylometricAnalyzer:
    ...
    def extract_readability_features(self, text: str) -> Dict[str, float]:
        ...
        try:
            # Correct textstat API calls
            features['flesch_kincaid_grade'] = float(textstat.flesch_kincaid_grade(text))
            features['gunning_fog_index'] = float(textstat.gunning_fog(text))
    ...
    def extract_academic_features(self, text: str) -> Dict[str, float]:
        ...
        words = re.findall(r'\b\w+\b', text.lower())
        if words:
            # Count words present in curated academic list
            awl_hits = sum(1 for word in words if word in self.academic_words)

            # Heuristic academic morphology: long words and common academic suffixes
            academic_suffixes = ('tion', 'sion', 'ment', 'ance', 'ence', 'ity', 'ology', 'graphy', 'istic', 'ative')
            heuristic_hits = 0
            for word in words:
                if len(word) >= 8 or word.endswith(academic_suffixes):
                    heuristic_hits += 1

            awl_ratio = awl_hits / len(words)
            heuristic_ratio = heuristic_hits / len(words)

            # Blend curated list with heuristic to avoid systematic zeros
            features['academic_word_ratio'] = min(1.0, 0.6 * awl_ratio + 0.4 * heuristic_ratio)
        ...
        # Passive voice detection (spaCy if available, else heuristic)
        sentences = re.split(r'[.!?]+', text)
        ...
        if self.nlp and HAS_SPACY:
            try:
                doc = self.nlp(text)
                # Count sentences that contain passive subject
                sent_has_passive = 0
                for sent in doc.sents:
                    if any(tok.dep_ in ('nsubjpass', 'auxpass') for tok in sent):
                        sent_has_passive += 1
                if sentences:
                    features['passive_voice_ratio'] = sent_has_passive / len(sentences)
            except Exception as e:
                logger.warning(f"spaCy passive detection failed: {e}")
        if features['passive_voice_ratio'] == 0.0:
            # Fallback heuristic
            passive_indicators = ['was', 'were', 'been', 'being']
            past_participle_endings = ('ed', 'en', 'ne')
            for sentence in sentences:
                s_lower = sentence.lower()
                if any(ind in s_lower for ind in passive_indicators):
                    sentence_words = re.findall(r'\b\w+\b', s_lower)
                    if any(w.endswith(past_participle_endings) for w in sentence_words):
                        passive_count += 1
            if sentences:
                features['passive_voice_ratio'] = passive_count / len(sentences)
    ...
    def extract_ai_detection_features(self, text: str) -> Dict[str, float]:
        ...
        # For single-sentence inputs, derive a non-zero repetition proxy from lexical diversity
        if len(sentences) <= 1 and words:
            unique = len(set(words))
            ttr = unique / len(words)
            # More repetition -> lower TTR; map to 0..1 where 1 means highly repetitive
            features['repetition_score'] = max(0.0, min(1.0, 1.0 - ttr))
