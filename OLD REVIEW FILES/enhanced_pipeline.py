...
class AcademicPlagiarismDetector:
    ...
    def __init__(self, corpus_index: CorpusIndex, use_domain_adaptation: bool = True):
        ...
        # Calibrated fusion weights (sum to 1.0)
        # Tuned for clearer separation of confident vs low-confidence cases
        if use_domain_adaptation:
            self.alpha_semantic = 0.6
            self.beta_cross = 0.25
            self.gamma_stylo = 0.15
        else:
            self.alpha_semantic = 0.7
            self.beta_cross = 0.0
            self.gamma_stylo = 0.3
    ...
    def analyze_sentence(self, sentence: str, top_k: int = 5) -> AcademicAnalysisResult:
        ...
        for match_text, semantic_similarity in raw_matches:
            ...
            # Calibrated fusion per configured weights
            if self.use_domain_adaptation:
                final_score = (
                    self.alpha_semantic * semantic_norm +
                    self.beta_cross * cross_encoder_score +
                    (1.0 - (self.alpha_semantic + self.beta_cross)) * 0.0
                )
            else:
        ...
        if processed_matches:
            ...
            if self.use_domain_adaptation:
                ...
                fused_score = (
                    self.alpha_semantic * max_semantic_score +
                    self.beta_cross * avg_cross_encoder_score +
                    self.gamma_stylo * stylometry_similarity
                )
            else:
                fused_score = (
        ...
        # Overall confidence assessment (slightly relaxed thresholds for clearer flags)
        if fused_score >= 0.75:
    ...
    def analyze_document(self, text: str) -> Dict[str, Any]:
        """Analyze entire document with document-level aggregation and summary."""
        ...
        # Calculate overall statistics and document-level summary
        if analyses:
            fused_scores = [self._to_float(a.fused_score) for a in analyses]
            semantic_scores = [self._to_float(a.semantic_score) for a in analyses]
            ...
            # Document-level score: average of top-K fused (captures concentrated plagiarism)
            top_k = min(10, len(fused_scores))
            top_k_avg = float(sum(sorted(fused_scores, reverse=True)[:top_k]) / max(top_k, 1))
            avg_fused = float(sum(fused_scores) / len(fused_scores))
            avg_semantic = float(sum(semantic_scores) / len(semantic_scores)) if semantic_scores else 0.0

            # Overall plagiarism score (0-1): balanced between top-k and average
            overall_plagiarism_score = 0.6 * top_k_avg + 0.4 * avg_fused
            ...
            overall_stats = {
                ...
                'overall_plagiarism_score': overall_plagiarism_score,
                'overall_plagiarism_percent': float(overall_plagiarism_score * 100.0),
            ...
            # Build top findings for quick review
            ranked = sorted(
                analyses,
                key=lambda a: self._to_float(a.fused_score),
                reverse=True
            )
            top_findings = []
            for a in ranked[:min(10, len(ranked))]:
                top_findings.append({
                    ...
                    'top_match': ({
                        'text': a.matches[0].text,
                        'similarity': float(a.matches[0].similarity),
                        'confidence': a.matches[0].confidence,
                    } if a.matches else None)
                })
        else:
            overall_stats = {
                'total_sentences': 0,
        ...
        return {
            'summary': {
                'overall_score': overall_stats.get('overall_plagiarism_score', 0.0),
                'overall_percent': overall_stats.get('overall_plagiarism_percent', 0.0),
                'high_risk_sentences': overall_stats.get('high_confidence_count', 0),
                'medium_risk_sentences': overall_stats.get('medium_confidence_count', 0),
                'low_risk_sentences': overall_stats.get('low_confidence_count', 0),
