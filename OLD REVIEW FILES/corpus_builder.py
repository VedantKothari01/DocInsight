...
class CorpusIndex:
    ...
    def __init__(self, target_size: int = 50000, cache_dir: str = "corpus_cache", use_domain_adaptation: bool = True):
        ...
        # Defer model initialization to loading/usage time to avoid repeated loads
    ...
    def is_ready_for_production(self) -> bool:
        ...
        ready_flag = Path(self.cache_dir / ".docinsight_ready")
        academic_ready_flag = Path(self.cache_dir / ".docinsight_academic_ready")
        return (
            self._is_fully_cached() and 
            (ready_flag.exists() or academic_ready_flag.exists())
        )
